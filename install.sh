...
# Copyright 2018- The datashack Authors.
#
...
CLOUD_ADDR=${PL_CLOUD_ADDR:-"datashack.io"}
DEFAULT_INSTALL_PATH=/usr/local/bin
ARTIFACT_NAME=cli_darwin_universal
USE_VERSION=${PL_CLI_VERSION:-latest}
USER_INSTALL_PATH="$HOME/bin"
ARTIFACT_BUCKET="datashack-dev-public"
if [[ $USE_VERSION == *"-"* ]]; then
  ARTIFACT_BUCKET="datashack-prod-artifacts"
fi
ARTIFACT_BASE_PATH="https://storage.googleapis.com/${ARTIFACT_BUCKET}/cli"


datashack_BANNER="
 ______   _______  _______  _______  _______  __   __  _______  _______  ___   _ 
|      | |   _   ||       ||   _   ||       ||  | |  ||   _   ||       ||   | | |
|  _    ||  |_|  ||_     _||  |_|  ||  _____||  |_|  ||  |_|  ||       ||   |_| |
| | |   ||       |  |   |  |       || |_____ |       ||       ||       ||      _|
| |_|   ||       |  |   |  |       ||_____  ||       ||       ||      _||     |_ 
|       ||   _   |  |   |  |   _   | _____| ||   _   ||   _   ||     |_ |    _  |
|______| |__| |__|  |___|  |__| |__||_______||__| |__||__| |__||_______||___| |_|                                                                                             
"
...
echo "${tty_green}${datashack_BANNER}${tty_reset}"

emph "Info:"
cat << EOS
This is POC for Datashack - Managed Data Platform for developers.
EOS

emph "Clonening the repo:"
git clone git@github.com:sh8kit/p0c1.git
cd p0c1

emph "Installing datashack CLI:"
pip install . --upgrade
echo 'Installation OK'

cd local_docker && make down && make up

declare -r HOST="http://localhost:8501"
...
echo
emph "Done. Next steps:"
cat << EOS
1. Write your model in "my_app/models" in python
2. run "datashack apply"
3. visit ${HOST} to see the datalake
EOS